...
public class upLoadBranUpdateProfileInfo extends AsyncTask<String, Void, String> {

    Context cxt;
    ProgressDialog progress ;

    public upLoadBranUpdateProfileInfo(Context context) {
        cxt=context;
    }
    ...
    protected String doInBackground(String... strings) {
       ...
       // Brand_image = strings[1];
        Brand_contact = strings[1];
        Brand_addres = strings[2];


        if (AppStatus.getInstance(cxt).isOnline()) {
            progress.show();
            String baseUrl = cxt.getResources().getString(R.string.baseUrl);
            String url_string = baseUrl+"/Neighbour/public/updateBrandInsertProfile";
            try {
                URL url = null;
                try {
